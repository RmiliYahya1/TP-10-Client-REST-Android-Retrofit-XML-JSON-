...
public class CompteRepository {
    ...
    public void getAllCompte(Callback<List<Compte>> callback) {
        if ("JSON".equals(format)) {
            Call<List<Compte>> call = compteService.getAllComptesJson();
            call.enqueue(callback);
        } else {
            Call<CompteList> call = compteService.getAllComptesXml();
            call.enqueue(new Callback<CompteList>() {
                ...
                public void onResponse(Call<CompteList> call, Response<CompteList> response) {
                    if (response.isSuccessful() && response.body() != null) {
                        ...
                        callback.onFailure(null, new Throwable("Erreur réponse XML"));
                    }
                }
